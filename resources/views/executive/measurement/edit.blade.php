...
<form class="form-horizontal"  id="edit_measurement_form" name="edit_measurement_form" action="{{ route('measurement_form_update_executive',$result->id) }}" method="POST" enctype="multipart/form-data">
					@csrf
						<div class="form-body">
    ...
    async function showAlert(mt_id) {
        ...
        .then((result) => {
            if (result.value) {
                ...
                $.ajax({
				url: '{{route('remove_measurement_item_executive')}}',
				type: 'GET',
				data: {mt_id: mt_id},
				success: function(data) {
                    ...
                    if(data.success == 'full'){
                            window.location.href = '{{ route('pending_order_list_executive') }}';
                        }
                        else{
                            reloadPage(); 
                        }
    ...
    var remarks = $('#remarks').val();
        ...
        if(type == 'Door With Frame'){
            if (model_id.trim() !== "" && frame_id.trim() !== "" && t_m_w.trim() !== "" && t_m_b_w.trim() !== "" && t_m_h.trim() !== "" && color_type.trim() !== "" && color_id.trim() !== "" && frame_color_id.trim() !== "") {
                $.ajax({
				url: '{{route('confirm_pending_order_executive')}}',
				type: 'GET',
...
<script>
    $(document).ready(function () {
        ...
        m_id = $('#m_id').val();
        $.ajax({
				url: '{{route('check_measurement_items_status_executive')}}',
				type: 'GET',
                data: {m_id: m_id},
				success: function(data) {
                    if(data.val == 0){
