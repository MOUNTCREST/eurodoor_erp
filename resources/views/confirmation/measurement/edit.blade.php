...
<form class="form-horizontal"  id="edit_measurement_form" name="edit_measurement_form" action="{{ route('measurement_form_update_confirmation',$result->id) }}" method="POST" enctype="multipart/form-data">
					@csrf
						<div class="form-body">
...
<select id="frame_pop_up_{{$no}}" name="frame_pop_up" class="form-input" onchange="return get_frame_size();">
                                                        ...
                                                        <option val="Wood" {{ $m_item->frame == 'Wood' ? 'selected' : '' }}>Wooden</option>
                                        ...
                                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4" id="mwc_div_{{$no}}">
                                          ...
                                          <div>
                                                ...
                                                <input type="number" readonly="readonly" step="any" id="m_w_c_w_pop_up_{{$no}}" name="m_w_c_w_pop_up" value="{{$m_item->measurement_with_clearance_top_width;}}" class="form-input">
                                            </div>
                                            <div>
                                                <label for="m_w_c_b_w_pop_up">Bottom Width<span style="color: #EB2D30"></span> </label>
                                                <input type="number" readonly="readonly" step="any" id="m_w_c_b_w_pop_up_{{$no}}" name="m_w_c_b_w_pop_up" value="{{$m_item->measurement_with_clearance_bottom_width;}}" class="form-input">
                                            </div>
                                            <div>
                                                <label for="m_w_c_h_pop_up">Height<span style="color: #EB2D30"></span> </label>
                                                <input type="number" readonly="readonly" step="any" id="m_w_c_h_pop_up_{{$no}}" name="m_w_c_h_pop_up" value="{{$m_item->measurement_with_clearance_height;}}" class="form-input">
                                            </div>
                                        </div>
                                        <hr class="my-3" style="border: 1px solid #b0b0b0 !important;"/>
                                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4" >
...
<script>
    ...
    function get_color_type_details(no){
                ...
                $.ajax({
                        url: '{{route('get_color_type_details_confirmation')}}',
                        type: 'GET',
...
<script>
    function get_clearence_top_width(no){
        frame_pop_up = $("#frame_pop_up_"+no).val();
        frame_size_pop_up = $("#frame_size_pop_up_"+no).val();
       // Get the values
        var t_m_w_pop_up = parseFloat($("#t_m_w_pop_up_"+no).val());
        var t_m_b_w_pop_up = parseFloat($("#t_m_b_w_pop_up_"+no).val());
        var t_m_h_pop_up = parseFloat($("#t_m_h_pop_up_"+no).val());
...
$.ajax({
        url: '{{route('get_top_width_clearence_door_only_confirmation')}}',
        type: 'GET',
        success: function(data) {
            ...
            $('#m_w_c_w_pop_up_'+no).val(total_width.toFixed(3));
        },
        error: function(jqXHR, textStatus, errorThrown) {
            // Your logic to handle the error
        }
            ...
            $('#m_w_c_w_pop_up_'+no).val(0);
            $('#m_w_c_b_w_pop_up_'+no).val(0);
            $('#m_w_c_h_pop_up_'+no).val(0);
        }   
        else{
        ...
        $.ajax({
                url: '{{route('get_top_width_clearence_confirmation')}}',
                type: 'GET',
...
<script>
    function get_clearence_bottom_width(no){
        frame_pop_up = $("#frame_pop_up_"+no).val();
        frame_size_pop_up = $("#frame_size_pop_up_"+no).val();
        // Get the values
        var t_m_w_pop_up = parseFloat($("#t_m_w_pop_up_"+no).val());
        ...
        $.ajax({
				...
				success: function(data) {
                    ...
                    $('#m_w_c_b_w_pop_up_'+no).val(total_width.toFixed(3));
				},
				error: function(jqXHR, textStatus, errorThrown) {
					// Your logic to handle the error
				}
...
<script>
    function get_clearence_height(no){
        frame_pop_up = $("#frame_pop_up_"+no).val();
        frame_size_pop_up = $("#frame_size_pop_up_"+no).val();
       // Get the values
        var t_m_w_pop_up = parseFloat($("#t_m_w_pop_up_"+no).val());
        ...
        success: function(data) {
            ...
            $('#m_w_c_h_pop_up_'+no).val(total_width.toFixed(3));
        },
        error: function(jqXHR, textStatus, errorThrown) {
            // Your logic to handle the error
        }
    ...
    async function showAlert(mt_id) {
        ...
        .then((result) => {
            if (result.value) {
                ...
                $.ajax({
				url: '{{route('remove_measurement_item_confirmation')}}',
				type: 'GET',
				data: {mt_id: mt_id},
				success: function(data) {
                 ...
                 //   reloadPage(); // Reload the page
                        ...
                        if(data.success == 'full'){
                            window.location.href = '{{ route('pending_order_list_confirmation') }}';
                        }
                        else{
                            reloadPage(); 
                        }
    ...
    var main_id = $('#main_id').val();
        ...
        if(type == 'Door With Frame'){
            if (model_id.trim() !== "" && frame_id.trim() !== "" && t_m_w.trim() !== "" && t_m_b_w.trim() !== "" && t_m_h.trim() !== "" && color_type.trim() !== "" && color_id.trim() !== "" && frame_color_id.trim() !== "") {
                $.ajax({
				url: '{{route('confirm_pending_order_confirmation')}}',
				type: 'GET',
...
<script>
  function get_door_color(no){
    color_type = $("#color_type_pop_up_"+no).val();
		...
		$.ajax({
				url: '{{route('get_color_list_confirmation')}}',
				type: 'GET',
...
function get_frame_size(){
    ...
    if(frame_pop_up == 'Fiber'){
        ...
        // $("#h_m_div").hide();
        $.ajax({
				url: '{{route('get_frame_size_deatils_confirmation')}}',
				type: 'GET',
				success: function(data) {
...
<script>
    $(document).ready(function () {
        ...
        m_id = $('#m_id').val();
        $.ajax({
				url: '{{route('check_measurement_items_status_confirmation')}}',
				type: 'GET',
                data: {m_id: m_id},
				success: function(data) {
                    if(data.val == 0){
  ...
  //   get_finsh_front_and_back();
    ...
    $.ajax({
				url: '{{route('get_customer_deatils_confirmation')}}',
				type: 'GET',
  ...
  $("#fitting_or_packing").change(function(){
    ...
    $.ajax({
				url: '{{route('generate_order_no_confirmation')}}',
				type: 'GET',
