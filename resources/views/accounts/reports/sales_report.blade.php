...
<div x-data="sorting">
<h5 class="mb-3 text-lg font-semibold dark:text-white-light md:top-[15px] md:mb-0">Sales Report</h5>
                        <div class="panel">
                  ...
                  <label>Customer</label>
                   <select id="customer_id" name="customer_id" class="form-input s2" >
                                                              <option  value="" selected>Select Customer</option>
                                                                @foreach($lg_lists as $row)
                                                                    <option value="{{$row->ledger_id}}">{{$row->customer_name}}</option>
                                                                @endforeach
                                                  </select>
                  </div>
									<div>
                ...
                <div x-data="exportTable">
                        <div class="panel">
                            ...
                            <tfoot align="right">
    <tr><th></th><th></th><th></th><th></th><th></th><th class="text-center">Total</th><th class="qty_total text-center"></th><th></th><th class="net_total text-center"></th><th></th></tr>
  </tfoot>
                            </table>
                        </div>
                    </div>
...
$("#btn_search").click(function () {
     ...
     $.ajax({
       url: '{{route('get_sales_report_in_account')}}',
       type: 'GET',
       data: {from_date:from_date,to_date:to_date,product_category_id:product_category_id,item_id:item_id,currency_id:currency_id,customer_id:customer_id},
       success: function (data) {
...
<script>
  function generatePDF() {
    ...
    doc.save('Sales Report-'+strDate+'-'+time+'.pdf');
  }
