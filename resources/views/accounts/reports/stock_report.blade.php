...
<div x-data="sorting">
<h5 class="mb-3 text-lg font-semibold dark:text-white-light md:top-[15px] md:mb-0">Stock Report</h5>
                        <div class="panel">
   ...
   <input type="date" id="to_date" name="to_date" class="form-input" autofocus='autofocus' value='<?php echo date("Y-m-d"); ?>' max='<?php echo date("Y-m-d"); ?>'>
                  ...
                  <select id="product_category_id" name="product_category_id" class="form-input" onchange="return get_items();" >
                ...
                <div x-data="exportTable">
						...
						<thead>
                            ...
                            <tfoot align="right">
    <tr><th></th><th></th><th></th><th></th><th></th><th class='text-center'>Total</th><th class="total_purchase_qty text-center"></th><th class="total_sales_qty text-center"></th><th class="total_balance text-center"></th></tr>
  </tfoot>
						</table>
                        </div>
                    </div>
...
$("#btn_search").click(function () {
        ...
        item_id = $("#item_id").val();
        $(".total_sales_qty").text("");
        $(".total_purchase_qty").text("");
        $(".total_balance").text("");
        sumcr =0;
        sumdr =0;
        sumbalance=0;
     ...
     $.ajax({
       url: '{{route('get_stock_report_in_account')}}',
       type: 'GET',
       data: { from_date: from_date,to_date:to_date,product_category_id:product_category_id,item_id:item_id },
     ...
     $.each(data, function (key, value) {
          ...
          dif = parseFloat(value[4]) - parseFloat(value[3]);
                        bal = parseFloat(bal) - parseFloat(dif);

                        prev_balance = (parseFloat(value[4])  - parseFloat(value[3])) + prev_balance;
                        
                        t_s_q = parseFloat(t_s_q) + parseFloat(value[3]);
                        t_p_q = parseFloat(t_p_q) + parseFloat(value[4]);
                        total_bal = parseFloat(t_p_q) - parseFloat(t_s_q);


                       

          const newRowData = [value[6],value[0],value[1],value[5],value[2],value[4],value[3],prev_balance.toFixed(3)];
          additems(newRowData);
          sl_no++;
          ...
          const totalRowData = ['','','','','<b>Total</b>', '<b>'+t_p_q.toFixed(3)+'</b>', '<b>'+t_s_q.toFixed(3)+'</b>', '<b>'+total_bal.toFixed(3)+'</b>'];
...
<script type="text/javascript">
  function get_items(){
     ...
     $.ajax({
				url: '{{route('get_item_list_in_account')}}',
				type: 'GET',
