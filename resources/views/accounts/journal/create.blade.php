...
<!-- start main content section -->
						<!-- <ul class="flex space-x-2 rtl:space-x-reverse">
                            <li>
                                <a href="{{ route('journal_list')}}" class="text-primary hover:underline">Journal</a>
                            </li>
                            <li class="before:content-['/'] ltr:before:mr-1 rtl:before:ml-1">
                                <span><a href="{{ route('journal_create')}}" class="text-primary hover:underline">Create</a></span>
                            </li>
                        </ul> -->
						...
						<div class="pX-5 panel w-full">
						  ...
						  <div class="container">
							 <div class="row">
							 <div class="col-sm-12">
							 <form class="form-horizontal"  id="add_journal" name="add_journal" action="{{ url('journal_account') }}" method="POST" enctype="multipart/form-data" onsubmit="return disableSubmitButton()">
					@csrf
						<div class="form-body">
							...
							<div class="form-group col-sm-3">
                                ...
                                <div class="form-group col-sm-3">
									<label for="t_date">Date<span style="color: #EB2D30">*</span> </label>
										<input type="date" id="t_date" max='<?php echo date("Y-m-d"); ?>' value="{{date('Y-m-d')}}" class="form-input" placeholder="" name="t_date" autocomplete="off">
										@error('t_date')
											<small class='text-danger'>{{ $message }}</small>
										@enderror
								</div> 
...
<script>
	function get_balance(){
        ...
        currency_id = $("#currency_id").val();
		 $.ajax({
				url: '{{route('get_total_balance_account')}}',
				type: 'GET',
...
$(document).ready(function(){
  $("#cr_ledger_id").change(function(){
	...
	currency_id = $("#currency_id").val(); 
	$.ajax({
				url: '{{route('get_total_credit_balance_account')}}',
				type: 'GET',
...
$(document).ready(function(){
  $("#dr_ledger_id").change(function(){
	...
	currency_id = $("#currency_id").val(); 
	$.ajax({
				url: '{{route('get_total_debit_balance_account')}}',
				type: 'GET',
