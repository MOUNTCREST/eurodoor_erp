...
<h5 class="mb-3 text-lg font-semibold dark:text-white-light md:top-[15px] md:mb-0">Production Pending List</h5>
                        <div class="panel">
							<div class="md:absolute md:top-5 ltr:md:left-5 rtl:md:right-5">
                                ...
                                <div>
									<label for="delivery_date">From Date<span style="color: #EB2D30"></span> </label>
										<input type="date" id="delivery_date" name="delivery_date" class="form-input">
									</div>
                                    <div>
									<label for="to_date">To Date<span style="color: #EB2D30"></span> </label>
										<input type="date" id="to_date" name="delivery_dato_datete" class="form-input">
									</div>
                                   
                                    <div>
									<label for="root_id">Fitting/Packing<span style="color: #EB2D30"></span> </label>
									...
									<label for="root_id">Root<span style="color: #EB2D30"></span> </label>
										<select id="root_id" name="root_id" class="form-input">
                                            <option  value="" selected>Select</option>
												...
												<option value="{{$row->id}}">{{$row->root_name}}</option>
											    @endforeach
                                        </select>
									</div>
									<div>
                        ...
                        <div class="panel">
                            ...
                            @foreach($mt_lists as $mt_list)
								<tr>
                                    ...
                                    <td>
                                    @if($mt_list->status === 'Available')
                                       <span class="text-success">{{$mt_list->status}}</span>
                                    @else
                                    <span class="text-danger">{{$mt_list->status}}</span>
                                    @endif
                                    </td>
									<td>
                                   
                                    @if($mt_list->company_id == Session::get('company_id'))
                                     @if($mt_list->status == 'Available')
                                    
									<div class="flex gap-1 items-center">
                                    <a href="{{ route('production_assigned_view', ['id' => $mt_list->mitems_id]) }}" class="hover:text-primary">
                                    ...
                                    <button class="btn btn-warning">Assign</button>
                                        ...
                                        @elseif($mt_list->order_type == 'Frame Only')
                                        <a href="{{ route('production_assign_frame_only', ['id' => $mt_list->mitems_id]) }}" class="hover:text-primary">
...
const refreshtbl = () => { // Use an arrow function to retain the context
    ...
    this.datatable.destroy(); // Destroy the existing datatable
    $('#myTable tbody').empty(); // Clear the tbody contents
    this.datatable.init(); // Reinitialize the datatable
...
$("#btn_search").click(function () {
     ...
     $.ajax({
       url: '{{route('get_production_pending_list_in_production')}}',
       type: 'GET',
       data: { delivery_date: delivery_date,to_date:to_date,fitting_or_packing: fitting_or_packing, root_id: root_id},
       success: function (data) {
        ...
        //   const newRowData = [value[0],value[1], value[2],value[3],value[4],value[5],value[6]];
        //   additems(newRowData);
        ...
        $.each(data, function (key, value) {
    const newRowData = [value[0], value[1], value[2], value[3], value[4], value[5], value[6]];
    var order_type = value[8];
  
    if (value[6] === 'Available') {
    const id = value[7];
    const route = "{{ route('production_assigned_view', ['id' => ':id']) }}".replace(':id', id);
    ...
    newRowData.push(linkButton[0].outerHTML); // Add the link button to the row data
}
else if(order_type == 'Frame Only')
    {
       
    const id = value[7];
    const route = "{{ route('production_assign_frame_only', ['id' => ':id']) }}".replace(':id', id);
    ...
    newRowData.push(linkButton[0].outerHTML); // Add the link button to the row data
        ...
        newRowData.push(regularButton[0].outerHTML); // Add the regular button to the row data
    }
...
<script>
  function generatePDF() {
    ...
    doc.save('Production Pending List '+ledger_name+'.pdf');
  }
