                        ...
                        <div class="panel">
                            ...
                            @foreach($mt_lists as $mt_list)
                                ...
                                <td>{{$no++}}</td>
                                    ...
                                    <td><a style="color:#1212c1;" href="{{ route('production_print_all', ['id' => $mt_list->id]) }}">{{$mt_list->batch_no}}</a></td>
                                    ...
                                    <div class="flex gap-1 items-center">
                                        <!-- <a href="{{ route('production_complete',$mt_list->mitems_id) }}"> -->
                                        <button type="button" class="btn btn-success ltr:ml-4 rtl:mr-4"  onclick ="return showAlert_confirm({{$mt_list->mitems_id}})">COMPLETE</button>
                                        <!-- </a> -->
    ...
    async function showAlert_confirm(mt_id) {
        ...
        .then((result) => {
            if (result.value) {
                ...
                $.ajax({
				url: '{{route('complete_production_pending_list_in_production')}}',
				type: 'GET',
				data: {mt_id: mt_id},
				success: function(data) {
                    swalWithBootstrapButtons.fire('Production Completed !', 'success').then(() => {
                ...
                swalWithBootstrapButtons.fire('Cancelled', 'Your Production file is safe :)', 'error');
            }
        });
    }
...
$("#btn_search").click(function () {
     ...
     $.ajax({
       ...
       type: 'GET',
       data: { delivery_date: delivery_date,fitting_or_packing: fitting_or_packing, root_id: root_id},
       success: function (data) {
...
<script>
  function generatePDF() {
    ...
    doc.save('Assigned List '+ledger_name+'.pdf');
  }
