...
<!-- start main content section -->
						...
						<form class="space-y-5"  id="edit_root" name="edit_root" action="{{ route('door_root_update',$result->id) }}" method="POST" enctype="multipart/form-data">
					@csrf
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
						<div>
						<label for="root_name">Root<span style="color: #EB2D30">*</span> </label>
										<input type="text" id="root_name" value="{{old('root_name') ? old('root_name') : $result->root_name }}" class="form-input" placeholder="" name="root_name" autocomplete="off">
										@error('root_name')
