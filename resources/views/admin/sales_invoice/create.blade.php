...
<form class="form-horizontal"  id="add_sales_invoice" name="add_sales_invoice" action="{{ url('sales_invoice') }}" method="POST" enctype="multipart/form-data" onsubmit="return disableSubmitButton()">
					@csrf
...
<script>
  function get_remarks(){
    ...
    $.ajax({
				url: '{{route('get_batch_nos_from_order_no')}}',
				type: 'GET',
...
$('#narration').text(narration);
  ...
  function get_details_frommeasurement_form(){
    order_no = $("#order_no").val();
    $.ajax({
				url: '{{route('get_details_from_measurement_form')}}',
				type: 'GET',
				data: {order_no: order_no},
				success: function(data) {
...
function get_tbl_creation(order_no){
  ...
  $.ajax({
				url: '{{route('get_m_items_for_sales')}}',
				type: 'GET',
				data: {order_no: order_no},
				success: function(data) {
