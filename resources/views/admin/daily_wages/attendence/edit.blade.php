...
<!-- start main content section -->
                        ...
                        <form class="form-horizontal"  id="edit_attendence" name="edit_attendence" action="{{ route('attendence_update',$result->id) }}" method="POST" enctype="multipart/form-data">
					@csrf
                    <div class="form-body">
                ...
                <div class="row">
                    <div class="form-group col-sm-3">
                        <label for="p_i_no">Date<span style="color: #EB2D30">*</span> </label>
                            <input type="date" id="added_date" value="{{$result->added_date}}" class="form-input"  placeholder="" name="added_date" autocomplete="off">
        @error('added_date')
                               ...
                               <table class="table table-bordered  table-striped text-center dataTables-example " id="tbl_items">
                                <thead>
                                   <tr>
                                    ...
                                    <tbody id="tbody">
                                    @foreach($al_items as $al_item)
                                      <tr id="R{{$no++}}">
                                        <td><h6>{{$no}}</h6></td>
                                        <td width="200">
                                          <select name="employee_id[]" class="form-input s2" id="employee_id{{$no}}"  required="required">
                                                  <option  value="" selected>Select</option>
                                                    @foreach ($ey_lists as $row): 
                                                        <option value="{{ $row->id }}" {{ $row->id == $al_item->employee_id ? 'selected' : '' }}>{{ $row->e_name }}</option>
                                                    @endforeach
                                         </select>
                                        </td>
                                        <td>
                                          <input type="time" class="form-input shift-time m_s_in" value="{{$al_item->moring_shift_in}}" name="m_s_in[]" autocomplete="off" id="m_s_in{{$no}}" onchange="return get_dif({{$no}});"  required="required">
                                        </td>
                                        
                                        <td>
                                          <input type="time" class="form-input shift-time m_s_out" value="{{$al_item->moring_shift_out}}" name="m_s_out[]" autocomplete="off" id="m_s_out{{$no}}" onchange="return get_dif({{$no}});"   required="required">
                                        </td>
                                        <td>
                                          <input type="time" class="form-input shift-time a_s_in" value="{{$al_item->after_shift_in}}" name="a_s_in[]" autocomplete="off" id="a_s_in{{$no}}"  onchange="return get_dif({{$no}});"  required="required">
                                        </td>
                                        <td>
                                          <input type="time" class="form-input shift-time a_s_out" value="{{$al_item->after_shift_out}}" name="a_s_out[]" autocomplete="off" id="a_s_out{{$no}}"  onchange="return get_dif({{$no}});"  required="required">
                                        </td>
                                        <td>
                                          <input type="text" class="form-input total" name="total[]" value="{{$al_item->total}}" autocomplete="off" id="total{{$no}}"  step="0.00001"   required="required">
                                          <input type="hidden" class="form-control form_element  p_h_amnt" name="p_h_amnt[]" autocomplete="off" id="p_h_amnt{{$no}}"  step="0.00001"   value="{{$al_item->per_hour_amount}}" required="required">
                                          <input type="hidden" class="form-control form_element  net_total_salary" name="net_total_salary[]" autocomplete="off" id="net_total_salary{{$no}}"  step="0.00001"   value="{{$al_item->total_salary}}" required="required">
                                        </td>
                                        <td>
              ...
              <line x1="6" y1="6" x2="18" y2="18"></line>
                                    ...
                                    <tfoot>
                                      <tr>
                                        ...
                                        <td><input type="text" class="form-input" value="{{$result->total}}"  name="total_h_m" id="total_h_m" step="0.00001" value="0"></td>
...
<script>
    $(document).ready(function () {
      ...
      $('#addBtn').on('click', function () {
        ...
        <td width="200">
                                                      <select name="employee_id[]" class="form-input s2" id="employee_id${rowIdx}"  required="required">
                                                              <option  value="" selected>Select</option>
                                                                @foreach ($ey_lists as $row): 
                                                    ...
                                                    <td>
                                                      ...
                                                      <input type="hidden" class="form-control form_element  p_h_amnt" name="p_h_amnt[]" autocomplete="off" id="p_h_amnt${rowIdx}"  step="0.00001"   value="0" required="required">
                                                      <input type="hidden" class="form-control form_element  net_total_salary" name="net_total_salary[]" autocomplete="off" id="net_total_salary${rowIdx}"  step="0.00001"   value="0" required="required">
                                                      </td>
                                                    <td>
                                                    <button type="button" class="remove" >
