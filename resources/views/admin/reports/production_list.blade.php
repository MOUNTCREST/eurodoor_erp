...
<div x-data="sorting">
<h5 class="mb-3 text-lg font-semibold dark:text-white-light md:top-[15px] md:mb-0">Production List</h5>
                        <div class="panel">
							<div class="md:absolute md:top-5 ltr:md:left-5 rtl:md:right-5">
								...
								<div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
									...
									<input type="date" name="d_date" id="d_date"  class="form-input" autofocus='autofocus' >
                                                                    ...
                                                                    <div>
									<label for="p_unit_id">Production Unit<span style="color: #EB2D30"></span> </label>
									<select id="p_unit_id" name="p_unit_id" class="form-input">
                                        <option  value="" selected>Select</option>
                                            @foreach ($ut_lists as $ut_list)
                                                <option value="{{ $ut_list->id }}">{{ $ut_list->production_unit_name }}</option>
                                            @endforeach
                                    </select>
                                									</div>
									<div>
...
$("#btn_search").click(function () {
    ...
    d_date = $("#d_date").val();
    p_unit_id = $("#p_unit_id").val();
     

     $.ajax({
       url: '{{route('get_report_of_production_list')}}',
       type: 'GET',
       data: { d_date: d_date,p_unit_id:p_unit_id },
       success: function (data) {
        ...
        if (data[0] === ''){
         ...
         $.each(data, function (key, value) {
          ...
          const newRowData = [value[0],value[1],value[2]];
          additems(newRowData);
