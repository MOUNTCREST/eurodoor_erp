...
<div x-data="">
<h5 class="mb-3 text-lg font-semibold dark:text-white-light md:top-[15px] md:mb-0">Ledger Report</h5>
                        <div class="panel">
...
$("#btn_search").click(function () {
    ...
    $.ajax({
        url: '{{route('get_ledger_report')}}',
        type: 'GET',
        data: { ledger_id: ledger_id, from_date: from_date, to_date: to_date },
        success: function (data) {
            refreshtbl();
           var dta = data.user_arr;
            if (dta[0] === ''){
          const newRowData = ["No entries found"];
          additems(newRowData);
} else {

            var opening_balance = data.balance;
            var array_of_ledger_transactions = data.user_arr;

            const openingBalanceRowData = ['', '', '', 'Opening Balance', 0, 0, opening_balance.toFixed(3)];
            additems(openingBalanceRowData);
            ...
            var prev_balance = opening_balance;

            // Add new rows to the DataTable
            $.each(array_of_ledger_transactions, function (key, value) {
                if (value[4] == 'cr') {
                    cr_amnt = value[3];
                    dr_amnt = nm.toFixed(3);
                    sum_cr = parseFloat(sum_cr) + parseFloat(cr_amnt);
                    s_cr = sum_cr;
                } else {
                    cr_amnt = nm.toFixed(3);
                    dr_amnt = value[3];
                    sum_dr = parseFloat(sum_dr) + parseFloat(dr_amnt);
                    s_dr = sum_dr;
                }
                sumcr = parseFloat(sumcr) + parseFloat(cr_amnt);
                sumdr = parseFloat(sumdr) + parseFloat(dr_amnt);

                t_bl = parseFloat(sumdr) - parseFloat(sumcr) + parseFloat(opening_balance);

                prev_balance = (parseFloat(dr_amnt) - parseFloat(cr_amnt)) + prev_balance;
                blnz_with_type = prev_balance.toFixed(3);

                const newRowData = [value[6], value[0], value[1], value[5], dr_amnt, cr_amnt, blnz_with_type];
                additems(newRowData);
                sl_no++;
            });
            ...
            $(".blnc").text(t_bl.toFixed(3));

            const totalRowData = ['','','','<b>Total</b>', '<b>'+sumdr.toFixed(3)+'</b>', '<b>'+sumcr.toFixed(3)+'</b>', '<b>'+t_bl.toFixed(3)+'</b>'];
