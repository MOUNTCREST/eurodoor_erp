...
<!-- start main content section -->
						...
						<div class="container">
							 <div class="row">
							 <div class="col-sm-12">
							 <form class="form-horizontal"  id="edit_payment" name="edit_payment" action="{{ route('payment_update',$result->account_id) }}" method="POST" enctype="multipart/form-data">
					@csrf
						<div class="form-body">
									...
									<label for="cr_ledger_id">From<span style="color: #EB2D30">*</span> </label>
                                        ...
                                        <option  value="" selected>Select</option>
                                            @foreach ($lg_lists_b_c as $row)
                                                <option value="{{ $row->id; }}" {{ ($row->id == $result_b->ledger_id || $row->id == old('cr_ledger_id')) ? 'selected' : '' }}>{{ $row->ledger; }}</option>
                                            @endforeach
                                        </select>
										@error('cr_ledger_id')
											<small class='text-danger'>{{ $message }}</small>
									...
									<label for="dr_ledger_id">To<span style="color: #EB2D30">*</span> </label>
                                        ...
                                        <option  value="" selected>Select</option>
                                            @foreach ($lg_lists as $row)
                                                <option value="{{ $row->id; }}" {{ ($row->id == $result_c->ledger_id || $row->id == old('dr_ledger_id')) ? 'selected' : '' }}>{{ $row->ledger; }}</option>
                                            @endforeach
                                        </select>
										@error('dr_ledger_id')
											<small class='text-danger'>{{ $message }}</small>
