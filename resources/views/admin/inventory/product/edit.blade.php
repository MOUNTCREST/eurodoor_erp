...
<!-- start main content section -->
						...
						<form class="space-y-5"  id="edit_product" name="edit_product" action="{{ route('product_update',$result->id) }}" method="POST" enctype="multipart/form-data">
					@csrf
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
						<div>
						<label for="product_name">Name<span style="color: #EB2D30">*</span> </label>
						<input type="text" id="product_name" value="{{old('product_name') ? old('product_name') : $result->product_name }}" class="form-input" placeholder="" name="product_name" autocomplete="off">
						@error('product_name')
						...
						<input type="text" id="barcode" value="{{old('barcode') ? old('barcode') : $result->barcode }}" class="form-input" placeholder="" name="barcode" autocomplete="off">
						</div>
						<div>
						...
						<option  value="" selected>Select</option>
							@foreach ($ut_lists as $row)
								<option value="{{ $row->id; }}" {{ $row->id == $result->unit_id ? 'selected' : '' }}>{{ $row->unit_name; }}</option>
							@endforeach
						</select>
						@error('unit_id')
							<small class='text-danger'>{{ $message }}</small>
						...
						<div>
						<label for="barcode">Quantity<span style="color: #EB2D30"></span> </label>
									<input type="number" step="0.00001" id="qty" value="{{$result->qty}}" class="form-control form_element" placeholder="" name="qty" autocomplete="off">
						</div>
					</div>
