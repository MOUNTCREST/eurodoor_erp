...
<!-- start main content section -->
						...
						<div class="container">
							 <div class="row">
							 <div class="col-sm-10">
							 <form class="form-horizontal"  id="add_mct" name="add_mct" action="{{ route('multi_currency_update',$result->account_id) }}" method="POST" enctype="multipart/form-data">
					@csrf
						<div class="form-body">
							...
							<div class="row">
                                ...
                                <div class="form-group col-sm-6">
									<label for="date">Date<span style="color: #EB2D30"></span> </label>
										<input type="date" id="date" value="{{$result->date}}" class="form-input" placeholder="" name="date" autocomplete="off">
									...
									<label for="account_dr">From<span style="color: #EB2D30"></span> </label>
                                        ...
                                        <option  value="" selected>Select</option>
                                            @foreach ($lg_lists as $row)
                                                <option value="{{ $row->id; }}" {{ $row->id == $result->account_dr ? 'selected' : '' }}>{{ $row->ledger; }}</option>
                                            @endforeach
                                        </select>
									...
									<label for="from_currency">From Currency<span style="color: #EB2D30"></span> </label>
                                        ...
                                        <option  value="" selected>Select</option>
                                            @foreach ($currency_lists as $row)
                                                <option value="{{ $row->id; }}" {{ $row->id == $result->from_currency ? 'selected' : '' }}>{{ $row->code; }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="form-group col-sm-6">
									<label for="account_cr">To<span style="color: #EB2D30"></span> </label>
                                        ...
                                        <option  value="" selected>Select</option>
                                            @foreach ($lg_lists as $row)
                                                <option value="{{ $row->id; }}" {{ $row->id == $result->account_cr ? 'selected' : '' }}>{{ $row->ledger; }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="form-group col-sm-6">
									<label for="to_currency">To Currency<span style="color: #EB2D30"></span> </label>
                                        ...
                                        <option  value="" selected>Select</option>
                                            @foreach ($currency_lists as $row)
                                                <option value="{{ $row->id; }}" {{ $row->id == $result->to_currency ? 'selected' : '' }}>{{ $row->code; }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="form-group col-sm-4">
									<label for="current_rate">Currenct Rate<span style="color: #EB2D30">*</span> </label>
										<input type="number" step="0.00001" id="current_rate" oninput="return calculation();" value="{{$result->current_rate}}" class="form-input" placeholder="" name="current_rate" autocomplete="off">
										@error('current_rate')
                                ...
                                <div class="form-group col-sm-4">
									<label for="from_amount">From Amount<span style="color: #EB2D30">*</span> </label>
										<input type="number" step="0.00001" id="from_amount" oninput="return calculation();" value="{{$result->from_amount}}" class="form-input" placeholder="" name="from_amount" autocomplete="off">
										@error('from_amount')
											<small class='text-danger'>{{ $message }}</small>
										@enderror
								</div> 
                                <div class="form-group col-sm-4">
									<label for="m_d_type">Type<span style="color: #EB2D30"></span> </label>
                                        ...
                                        <option  value="" selected>Select</option>
                                        <option  value="DV"  {{ $result->m_d_type == 'DV' ? 'selected' : '' }}>Division</option>
                                        <option  value="MT"  {{ $result->m_d_type == 'MT' ? 'selected' : '' }}>Multiplication</option>
                                        </select>
								</div> 
                                <div class="form-group col-sm-4">
									<label for="to_amount">To Amount<span style="color: #EB2D30">*</span> </label>
										<input type="number" step="0.00001" id="to_amount" value="{{$result->to_amount}}" class="form-input" placeholder="" name="to_amount" autocomplete="off">
										@error('to_amount')
											<small class='text-danger'>{{ $message }}</small>
										@enderror
								</div> 
