...
<form class="form-horizontal"  id="edit_measurement_form" name="edit_measurement_form" action="{{ route('measurement_form_update',$result->id) }}" method="POST" enctype="multipart/form-data">
					@csrf
						<div class="form-body">
    ...
    var main_id = $('#main_id').val();
        ...
        if(type == 'Door With Frame'){
            if (model_id.trim() !== "" && frame_id.trim() !== "" && t_m_w.trim() !== "" && t_m_b_w.trim() !== "" && t_m_h.trim() !== "" && color_type.trim() !== "" && color_id.trim() !== "" && frame_color_id.trim() !== "") {
                $.ajax({
				url: '{{route('confirm_pending_order')}}',
				type: 'GET',
