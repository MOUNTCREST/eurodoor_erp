...
<form class="form-horizontal"  id="edit_measurement_form" name="edit_measurement_form" action="{{ route('measurement_form_update_packing',$result->id) }}" method="POST" enctype="multipart/form-data">
					@csrf
						<div class="form-body">
...
<script>
    ...
    async function showAlert(mt_id) {
        ...
        .then((result) => {
            if (result.value) {
                ...
                $.ajax({
				url: '{{route('remove_measurement_item_packing')}}',
				type: 'GET',
				data: {mt_id: mt_id},
				success: function(data) {
                    ...
                    if(data.success == 'full'){
                            window.location.href = '{{ route('pending_order_list_packing') }}';
                        }
                        else{
                            reloadPage(); 
                        }
    ...
    var main_id = $('#main_id').val();
        ...
        if(type == 'Door With Frame'){
            if (model_id.trim() !== "Select" && frame_id.trim() !== "" && t_m_w.trim() !== "" && t_m_b_w.trim() !== "" && t_m_h.trim() !== "" && color_type.trim() !== "" && color_id.trim() !== "" && frame_color_id.trim() !== "") {
                $.ajax({
				url: '{{route('confirm_pending_order_packing')}}',
				type: 'GET',
...
<script>
    $(document).ready(function () {
        ...
        m_id = $('#m_id').val();
        $.ajax({
				url: '{{route('check_measurement_items_status_packing')}}',
				type: 'GET',
                data: {m_id: m_id},
				success: function(data) {
                    if(data.val == 0){
