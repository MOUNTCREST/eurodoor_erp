...
<form class="form-horizontal"  id="add_measurement_form" name="add_measurement_form" action="{{ url('measurement_form_fitting') }}" method="POST" enctype="multipart/form-data" onsubmit="return disableSubmitButton()">
					@csrf
						<div class="form-body">
                            ...
                            <label for="fitting_or_packing">Fitting <span style="color: #EB2D30">*</span> </label>
                                        <input type="text" id="fitting_or_packing"  class="form-input" placeholder=" " value="Fitting"  name="fitting_or_packing" readonly="readonly" autocomplete="off">
								</div> 
								<div class="form-group col-sm-3">
...
<script>
    function get_color_type_and_color_by_model(){
        ...
        $.ajax({
				url: '{{route('get_color_type_and_color_name_fitting')}}',
...
function get_sub_models(){
    ...
    model_id = $("#model_pop_up").val();
		...
		$.ajax({
				url: '{{route('get_sub_models_fitting')}}',
				type: 'GET',
				data: {model_id: model_id},
				success: function(data) {
...
function get_details_of_sub_model(){
    sub_model_id = $("#sub_model_pop_up").val();
		...
		$.ajax({
				url: '{{route('get_details_of_sub_model_fitting')}}',
				type: 'GET',
...
<script>
    $(document).ready(function () {
     ...
     $("#lock_measurement_div").show();
       ...
       $('#frame_pop_up').append('<option value=" ">Select</option>');
       $('#frame_pop_up').append('<option value="Concrete">Concrete</option>');
       $('#frame_pop_up').append('<option value="Wood">Wood</option>');
       $('#frame_pop_up').append('<option value="Fiber">Fiber</option>');
       $('#frame_pop_up').append('<option value="Stainless Steel">Stainless Steel</option>');
...
<script>
    function check_items(order_no){
        $.ajax({
				url: '{{route('check_in_measurement_items_fitting')}}',
				type: 'GET',
...
<script>
    ...
    <script>
        ...
        $.ajax({
				url: '{{route('save_as_tem_data_m_items_fitting')}}',
				type: 'GET',
...
$('#m_item_tbl tbody').on('click', '.delete-btn', function() {
    ...
    $.ajax({
				url: '{{route('delete_m_temp_item_fitting')}}',
				type: 'GET',
				data: {itemId: itemId},
...
<script>
    ...
    $.ajax({
				url: '{{route('get_list_of_m_items_fitting')}}',
				type: 'GET',
				data: {order_no: order_no},
				success: function(response) {
                    // Remove the row from the table if the deletion was successful
