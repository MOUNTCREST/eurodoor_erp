...
class Measurement extends Controller
{
    ...
    public function index()
    {
		...
		return view('executive.measurement.index');
    }
    public function get_pending_order_list_executive(){
        ...
        ->where("measurements.department",'executive')
        ->get();
    ...
    foreach ($result as $r) {
        ...
        $editButton = '<a href="' . route('measurement_form_edit_executive',$r->id ) . '"> <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-4.5 h-4.5">
        <path
            opacity="0.5"
    ...
    $viewButton = '<a href="' . route('measurment_form_view_executive',$r->id ) . '">  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
    <path
        opacity="0.5"
    ...
    public function create()
    {
        ...
        return view('executive.measurement.create',$data)->with('no', 1);
    }
    ...
    public function store(Request $request)
    {
        $validatedData = $request->validate([
            ...
            'executive_or_executive' => 'required',
            'order_date' => 'required',
        ...
        $m_id = DB::connection('mysql')->table('measurements')->insertGetId([
            'executive_or_executive' =>  request('executive_or_executive'),
        ...
        DB::connection('mysql_second')->table('measurements')->insert([
         ...
         return redirect(route('pending_order_list_executive'));
    }
    ...
    public function show(string $id)
    {
        ...
        return view('executive.measurement.completed_order_view',compact('result','m_items'))->with('no', 1);
    }
    ...
    public function edit(string $id)
    {
        ...
        return view('executive.measurement.edit',compact('result','door_thickness_lists','currency_lists','m_items','customer_lists','executive_lists','model_lists','color_lists','root_lists','gt_lists','lk_lists','fs_lists','single_color_list','brnd_lists'))->with('no', 0);
    ...
    public function update(Request $request, string $id)
    {
        ...
        DB::connection('mysql_second')->table('measurements')->where('id', $id)
            ...
            return redirect(route('confirmed_order_list_executive'));
    }
    ...
    public function generate_order_no(Request $request){
        $executive_or_executive = $request->executive_or_executive;
         ...
         if($executive_or_executive == "executive"){
            $data_mi_executive =  DB::table('measurements')
        ->select('measurements.id')
        ->where("measurements.company_id", "=", Session::get('company_id'))
        ->where("measurements.executive_or_executive", "=", "executive")
        ->get();
    

         if(empty($data_mi_executive)){
            $num = 0;
         }
         else{
         ...
         $refe_executive = $num + 1;
         $ref_no_executive = str_pad($refe_executive, 4, '0', STR_PAD_LEFT);
         $r_no = "FI".$ref_no_executive;
         }
         else{
            $data_mi_executive =  DB::table('measurements')
        ->select('measurements.id')
        ...
        ->get();
         ...
         $refe_executive = $num_p + 1;
         $ref_no_executive = str_pad($refe_executive, 4, '0', STR_PAD_LEFT);
         $r_no = "PC".$ref_no_executive;
         }
    ...
    public function confirm_pending_order(Request $request){
        ...
        DB::connection('mysql')->table('measurements')->where('id', $main_id)
            ->update([
                'executive_or_executive' =>  $executive_or_executive,
    ...
    public function confirmed_order_list(){
		...
		return view('executive.measurement.confirmed_order_list'); 
    
    }
    public function get_confirmed_order_list_executive(){
        $draw = request()->input('draw');
        $start = request()->input('start');
        $length = request()->input('length');
    ...
    $viewButton = '<a href="' . route('completed_order_view',$r->id ) . '">  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
    <path
        opacity="0.5"
        d="M3.27489 15.2957C2.42496 14.1915 2 13.6394 2 12C2 10.3606 2.42496 9.80853 3.27489 8.70433C4.97196 6.49956 7.81811 4 12 4C16.1819 4 19.028 6.49956 20.7251 8.70433C21.575 9.80853 22 10.3606 22 12C22 13.6394 21.575 14.1915 20.7251 15.2957C19.028 17.5004 16.1819 20 12 20C7.81811 20 4.97196 17.5004 3.27489 15.2957Z"
        stroke="currentColor"
   ...
   $date = \Carbon\Carbon::parse($r->order_date)->format('d-m-Y') ;
    ...
    public function completed_order_edit(string $id){
        ...
        return view('executive.measurement.completed_order_edit',compact('result','m_items','customer_lists','executive_lists','model_lists','color_lists','root_lists','gt_lists','lk_lists','fs_lists'))->with('no', 0);
    }
    ...
    public function measurment_form_view(string $id){
        ...
        return view('executive.measurement.measurment_form_view',compact('result','m_items'))->with('no', 1);
    }
...
$rowCount += 1;
    ...
    public function executive_change_password(){
        return view('executive.change_password'); 
    }
   
    public function change_password_executive(Request $request, string $id)
    {
        $validatedData = $request->validate([
    ...
    DB::connection('mysql_second')->table('users')->where('id', $id)
        ...
        return redirect()->route('executive_change_password')->with('success', 'Password changed successfully.');
    
}
public function get_top_width_clearence_door_only_executive(Request $request){
    $details = DB::table('doorclearances')
    ->select('doorclearances.*')
    ->where("company_id", "=", Session::get('company_id'))
    ->where("delete_status", "=", 0)
