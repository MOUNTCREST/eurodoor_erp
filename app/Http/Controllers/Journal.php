...
class Journal extends Controller
{
  ...
  foreach ($result as $r) {
      ...
      $editButton = '<a href="' . route('journal_edit',$r->id ) . '"> <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-4.5 h-4.5">
      <path
          opacity="0.5"
  ...
  $viewButton = '<a href="' . route('journal_view',$r->id ) . '">  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
  <path
      opacity="0.5"
  ...
  $btns = '<form method="post" action="'. route('journal_delete', $r->account_id) .'">'. csrf_field() . method_field('DELETE') .'<div class="flex gap-4 items-center">'.$editButton.$viewButton.$deleteButton.'</div></form>';

 $date = \Carbon\Carbon::parse($r->added_date)->format('d-m-Y') ;
 $ref_no ='<a style="color:#1212c1;" href="'. route('journal_view', $r->id) .'">'.$r->reference_no.'</a>';
      $data[] = array(
          $slno,
          $date,
          $ref_no,
    ...
    public function create()
    {
        ...
        return view('admin.accounts.journal.create',$data);
    }
   ...
   DB::connection('mysql_second')->table('accounts')->insert([
     ...
     DB::connection('mysql_second')->table('black_or_whites')->insert([
       ...
       'updated_at' => DB::raw('NOW()'),
        ...
        return redirect(route('journal_list'));
    }
    ...
    public function show(string $id)
    {
      ...
      return view('admin.accounts.journal.view',compact('result','t_details'))->with('no', 1);
    }
    ...
    public function edit(jrnl $id)
    {
        ...
        return view('admin.accounts.journal.edit',compact('result','lg_lists','currency_lists','result_a','result_b','result_c','c_balance'));
    }
    ...
    if(request('type') == 'black'){
     ...
     DB::connection('mysql')->table('accounts')->where('id', $id)
         ...
         // Find the record to update in database1
     $record1 = DB::connection('mysql')->table('journals')->where('account_id', $account_id)->first();
  
     // Update the record with the new data
     DB::connection('mysql')->table('journals')->where('account_id', $account_id)
         ->update([
           'reference_no' =>  request('reference_no'),
           'fitting_or_packing' => request('fitting_or_packing'),
           'account_id' => $account_id,
     ...
     // Find the record to update in database2
     $record2 = DB::connection('mysql_second')->table('journals')->where('account_id', $account_id)->first();
  
     // Update the record with the new data
     DB::connection('mysql_second')->table('journals')->where('account_id', $id)
         ->update([
           'reference_no' =>  request('reference_no'),
           'fitting_or_packing' => request('fitting_or_packing'),
           'account_id' => $account_id,
  ...
  DB::connection('mysql_second')->table('account_transactions')->where('account_id', $id)
      ->update([
       ...
       // Find the record to update in database1
  $record5 = DB::connection('mysql')->table('journals')->where('account_id', $id)->first();

  // Update the record with the new data
  DB::connection('mysql')->table('journals')->where('account_id', $id)
      ->update([
          'delete_status' => 1,
  ...
  // Find the record to update in database2
  $record6 = DB::connection('mysql_second')->table('journals')->where('account_id', $id)->first();
  ...
  DB::connection('mysql_second')->table('black_or_whites')->where('account_id', $id)
    ...
    public function journal_pending(){
		return view('admin.admin_approvel.journal_pending.index');
    }
    public function get_journal_pending_list(){
      $draw = request()->input('draw');
      $start = request()->input('start');
      $length = request()->input('length');
  ...
  foreach ($result as $r) {
      ...
      $editButton = '<a href="' . route('journal_pending_edit',$r->id ) . '"> <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-4.5 h-4.5">
      <path
          opacity="0.5"
          d="M22 10.5V12C22 16.714 22 19.0711 20.5355 20.5355C19.0711 22 16.714 22 12 22C7.28595 22 4.92893 22 3.46447 20.5355C2 19.0711 2 16.714 2 12C2 7.28595 2 4.92893 3.46447 3.46447C4.92893 2 7.28595 2 12 2H13.5"
          stroke="currentColor"
  ...
  $viewButton = '<a href="' . route('journal_pending_view',$r->id ) . '">  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
  <path
      opacity="0.5"
      d="M3.27489 15.2957C2.42496 14.1915 2 13.6394 2 12C2 10.3606 2.42496 9.80853 3.27489 8.70433C4.97196 6.49956 7.81811 4 12 4C16.1819 4 19.028 6.49956 20.7251 8.70433C21.575 9.80853 22 10.3606 22 12C22 13.6394 21.575 14.1915 20.7251 15.2957C19.028 17.5004 16.1819 20 12 20C7.81811 20 4.97196 17.5004 3.27489 15.2957Z"
      stroke="currentColor"
  ...
  $btns = '<form method="post" action="'. route('journal_pending_delete', $r->account_id) .'">'. csrf_field() . method_field('DELETE') .'<div class="flex gap-4 items-center">'.$editButton.$viewButton.$deleteButton.'</div></form>';

 $date = \Carbon\Carbon::parse($r->added_date)->format('d-m-Y') ;
 $ref_no ='<a style="color:#1212c1;" href="'. route('journal_pending_view', $r->id) .'">'.$r->reference_no.'</a>';
      $data[] = array(
          $slno,
          $date,
          $ref_no,
    ...
    public function journal_pending_view(string $id){
      ...
      return view('admin.admin_approvel.journal_pending.view',compact('result'))->with('no', 1);
  }
...
DB::connection('mysql_second')->table('journals')->where('id', $id)
  ...
  return redirect(route('journal_pending'));
  }
  ...
  public function journal_pending_edit(jrnl $id)
  {
      ...
      return view('admin.admin_approvel.journal_pending.edit',compact('result','lg_lists','currency_lists','c_balance'));
  }
