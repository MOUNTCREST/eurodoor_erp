...
class Journal extends Controller
{
    ...
    {
		return view('packing.accounts.journal.index');
    }
public function get_journal_list_packing(){
  $draw = request()->input('draw');
      ...
      ->where("journals.department", "=",'Packing')
      ->get();
  ...
  foreach ($result as $r) {
      ...
      $editButton = '<a href="' . route('journal_packing_edit',$r->id ) . '"> <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-4.5 h-4.5">
      <path
          opacity="0.5"
  ...
  $btns = '<form method="post" action="'. route('journal_packing_delete', $r->id) .'">'. csrf_field() . method_field('DELETE') .'<div class="flex gap-4 items-center">'.$editButton.$deleteButton.'</div></form>';
 ...
 $ref_no =$r->reference_no;
    ...
    public function create()
    {
        ...
        return view('packing.accounts.journal.create',$data);
    }
...
DB::connection('mysql_second')->table('journals')->insert([
 ...
 'updated_at' => DB::raw('NOW()'),
    ...
    public function show(string $id)
    {
      ...
      return view('packing.accounts.journal.view',compact('result','t_details'))->with('no', 1);
    }
    ...
    public function edit(jrnl $id)
    {
        ...
        return view('packing.accounts.journal.edit',compact('result','lg_lists','currency_lists'));
    }
