...
class Doormodel extends Controller
{
    /**
    ...
    public function index()
    {
		...
		return view('admin.door_masters.door_model.index');
    }
    ...
    foreach ($result as $r) {
        ...
        $editButton = '<a href="' . route('door_model_edit',$r->id ) . '"> <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-4.5 h-4.5">
        <path
            opacity="0.5"
    ...
    $btns = '<form method="post" action="'. route('model_delete', $r->id) .'">'. csrf_field() . method_field('DELETE') .'<div class="flex gap-4 items-center">'.$editButton.$deleteButton.'</div></form>';
    ...
    {
        $data_dm =  DB::table('doormodels')
        ->select('doormodels.id')
        ...
        ->get();
         ...
         $r_no = "DM-".$ref_no;
         $data = ['r_no' => $r_no];
        return view('admin.door_masters.door_model.create',$data);
    }
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () {
            DB::connection('mysql')->table('doormodels')->insert([
            ...
            DB::connection('mysql_second')->table('doormodels')->insert([
                'model_name' =>  request('model_name'),
                'ref_no' => request('ref_no'),
                'die_type' => request('die_type'),
                'color_type' => request('color_type'),
        ...
        return redirect(route('door_model_list'));
    }
    ...
    public function edit(dml $id)
    {
        ...
        $result = $id;
        return view('admin.door_masters.door_model.edit',compact('result','color_lists'));
    }
    ...
    $record1 = DB::connection('mysql')->table('doormodels')->where('id', $id)->first();

    // Update the record with the new data
    DB::connection('mysql')->table('doormodels')->where('id', $id)
        ->update([
            'model_name' =>  request('model_name'),
            'ref_no' => request('ref_no'),
            'die_type' => request('die_type'),
    ...
    $record2 = DB::connection('mysql_second')->table('doormodels')->where('id', $id)->first();

    // Update the record with the new data
    DB::connection('mysql_second')->table('doormodels')->where('id', $id)
        ->update([
            'model_name' =>  request('model_name'),
            'ref_no' => request('ref_no'),
            'die_type' => request('die_type'),
