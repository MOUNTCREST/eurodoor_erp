...
class Contra extends Controller
{
    ...
    {
		return view('fitting.accounts.contra.index'); 
    }
public function get_contra_list_fitting(){
  ...
  ->where("contras.department", "=",'Fitting')
  ->get();
...
foreach ($result as $r) {
  ...
  $editButton = '<a href="' . route('contra_fitting_edit',$r->id ) . '"> <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-4.5 h-4.5">
  <path
      opacity="0.5"
...
$btns = '<form method="post" action="'. route('contra_fitting_delete', $r->id) .'">'. csrf_field() . method_field('DELETE') .'<div class="flex gap-4 items-center">'.$editButton.$deleteButton.'</div></form>';
...
$ref_no =$r->reference_no;
  ...
  exit();
    ...
    public function create()
    {
        ...
        return view('fitting.accounts.contra.create',$data);
    }
    ...
    public function store(Request $request)
    {
      ...
      'fitting_or_packing.required' => 'The Fitting Or Packing field is required.',
        ...
        return redirect(route('contra_fitting_list'));
    }
    ...
    public function show(string $id)
    {
      ...
      return view('accounts.contra.view',compact('result'))->with('no', 1);
    }
    ...
    public function edit(cnta $id)
    {
        ...
        return view('fitting.accounts.contra.edit',compact('result','lg_lists_b_c','currency_lists'));
    }
    ...
    public function destroy(string $id)
    {
       ...
       // Find the record to update in database1
  $record5 = DB::connection('mysql')->table('contras')->where('id', $id)->first();
  ...
  DB::connection('mysql_second')->table('contras')->where('id', $id)
    ...
    public function contra_pending(){
		...
		$data = ['contra_lists'=> $contra_lists];
		return view('admin_approvel.contra_pending.index',$data)->with('no', 1); 
    }
    public function contra_pending_view(string $id){
        ...
        return view('admin_approvel.contra_pending.view',compact('result'))->with('no', 1);
    }
    public function contra_approve(string $id){
        ...
        DB::connection('mysql_second')->table('contras')->where('id', $id)
          ...
          DB::connection('mysql_second')->table('accounts')->insert([
            ...
            return redirect(route('contra_packing_list'));
      ...
      public function contra_pending_edit(cnta $id)
    {
        ...
        return view('admin_approvel.contra_pending.edit',compact('result','lg_lists_b_c','currency_lists','c_balance'));
    }
