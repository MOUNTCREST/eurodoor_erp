...
class Receipt extends Controller
{
    ...
    {
		return view('fitting.accounts.receipt.index');  
    }
public function get_receipt_list_fitting(){
    ...
    ->where("receipts.department", "=",'Fitting')
    ->get();
...
foreach ($result as $r) {
    ...
    $editButton = '<a href="' . route('receipt_fitting_edit',$r->id ) . '"> <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-4.5 h-4.5">
    <path
        opacity="0.5"
...
$btns = '<form method="post" action="'. route('receipt_fitting_delete', $r->id) .'">'. csrf_field() . method_field('DELETE') .'<div class="flex gap-4 items-center">'.$editButton.$deleteButton.'</div></form>';
...
$ref_no =$r->reference_no;
    ...
    public function create()
    {
        ...
        return view('fitting.accounts.receipt.create',$data);
    }
  ...
  if(request('type') == 'black'){
   ...
   DB::connection('mysql_second')->table('receipts')->insert([
     ...
     'updated_at' => DB::raw('NOW()'),
        ...
        return redirect(route('receipt_fitting_list'));
    }
    ...
    public function show(string $id)
    {
      ...
      return view('accounts.receipt.view',compact('result','t_details'))->with('no', 1);
    }
    ...
    public function edit(rcpt $id)
    {
        ...
        return view('fitting.accounts.receipt.edit',compact('result','lg_lists','lg_lists_b_c','currency_lists'));
...
DB::connection('mysql_second')->table('receipts')->where('id', $id)
    ...
    public function destroy(string $id)
    {
       ...
       // Find the record to update in database1
  $record5 = DB::connection('mysql')->table('receipts')->where('id', $id)->first();
  ...
  $data = ['receipt_lists'=> $receipt_lists];
  return view('admin_approvel.receipt_pending.index',$data)->with('no', 1);  
    }
    public function receipt_pending_view(string $id){
      ...
      return view('admin_approvel.receipt_pending.view',compact('result'))->with('no', 1);
  }
  public function receipt_approve(string $id){
     ...
     DB::connection('mysql_second')->table('receipts')->where('id', $id)
       ...
       DB::connection('mysql_second')->table('accounts')->insert([
         ...
         return redirect(route('receipt_packing_list'));
  }
...
DB::connection('mysql_second')->table('receipts')->where('id', $id)
  ...
  public function receipt_pending_edit(rcpt $id){
    $lg_lists =  DB::table('account_ledgers')
        ...
        return view('admin_approvel.receipt_pending.edit',compact('result','lg_lists','lg_lists_b_c','currency_lists','c_balance'));
  }
  public function receipt_pending_update(string $id){
    // Find the record to update in database1
    $record1 = DB::connection('mysql')->table('receipts')->where('id', $id)->first();
