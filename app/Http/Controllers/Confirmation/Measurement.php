...
class Measurement extends Controller
{
    ...
    {
		return view('confirmation.measurement.index'); 
    }
    public function get_pending_order_list_confirmation(){
    ...
    foreach ($result as $r) {
        ...
        $editButton = '<a href="' . route('measurement_form_edit_confirmation',$r->id ) . '"> <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-4.5 h-4.5">
        <path
            opacity="0.5"
    ...
    $viewButton = '<a href="' . route('measurment_form_view_confirmation',$r->id ) . '">  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
    <path
        opacity="0.5"
  ...
  /**
    ...
    public function show(string $id)
    {
        ...
        return view('confirmation.measurement.completed_order_view',compact('result','m_items'))->with('no', 1);
    }
    ...
    public function edit(string $id)
    {
        ...
        return view('confirmation.measurement.edit',compact('result','door_thickness_lists','currency_lists','m_items','customer_lists','executive_lists','model_lists','color_lists','root_lists','gt_lists','lk_lists','fs_lists','single_color_list','brnd_lists'))->with('no', 0);
    ...
    public function update(Request $request, string $id)
    {
        ...
        DB::connection('mysql_second')->table('measurements')->where('id', $id)
            ...
            return redirect(route('confirmed_order_list_confirmation'));
    }
    ...
    public function generate_order_no(Request $request){
        ...
        ->get();
         ...
         $r_no = "FI".$ref_no_fitting;
            ...
            $data_mi_packing =  DB::table('measurements')
        ->select('measurements.id')
        ->where("measurements.company_id", "=", Session::get('company_id'))
        ->where("measurements.fitting_or_packing", "=", "Packing")
        ->get();
    

         if(empty($data_mi_packing)){
            $num_p = 0;
         }
         else{
         ...
         $refe_packing = $num_p + 1;
         $ref_no_packing = str_pad($refe_packing, 4, '0', STR_PAD_LEFT);
         $r_no = "PC".$ref_no_packing;
         }
    ...
    public function confirmed_order_list(){
		...
		return view('confirmation.measurement.confirmed_order_list'); 
    
    }
    public function get_confirmed_order_list_confirmation(){
        $draw = request()->input('draw');
        $start = request()->input('start');
        $length = request()->input('length');
    ...
    $viewButton = '<a href="' . route('completed_order_view_confirmation',$r->id ) . '">  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
    <path
        opacity="0.5"
        d="M3.27489 15.2957C2.42496 14.1915 2 13.6394 2 12C2 10.3606 2.42496 9.80853 3.27489 8.70433C4.97196 6.49956 7.81811 4 12 4C16.1819 4 19.028 6.49956 20.7251 8.70433C21.575 9.80853 22 10.3606 22 12C22 13.6394 21.575 14.1915 20.7251 15.2957C19.028 17.5004 16.1819 20 12 20C7.81811 20 4.97196 17.5004 3.27489 15.2957Z"
        stroke="currentColor"
   ...
   $date = \Carbon\Carbon::parse($r->order_date)->format('d-m-Y') ;
    ...
    public function completed_order_edit(string $id){
        ...
        return view('confirmation.measurement.completed_order_edit',compact('result','m_items','customer_lists','executive_lists','model_lists','color_lists','root_lists','gt_lists','lk_lists','fs_lists'))->with('no', 0);
    }
    ...
    public function measurment_form_view(string $id){
        ...
        return view('confirmation.measurement.measurment_form_view',compact('result','m_items'))->with('no', 1);
    }
...
$rowCount += 1;
    ...
    public function change_password_confirmation(Request $request, string $id)
    {
        $validatedData = $request->validate([
...
public function get_pending_order_by_fp(Request $request){
    $fitting_or_packing = trim($request->fitting_or_packing);
    ...
    ->where("measurements.company_id", "=", Session::get('company_id'))
    ->where("measurements.status", "=", 'Pending')
    ->where('measurements.fitting_or_packing',"=" ,$fitting_or_packing)
    ->get();
...
public function get_confirmed_order_by_fp(Request $request){
    $fitting_or_packing = trim($request->fitting_or_packing);
...
public function get_top_width_clearence_door_only(Request $request){
    $details = DB::table('doorclearances')
    ->select('doorclearances.*')
    ->where("company_id", "=", Session::get('company_id'))
    ->where("delete_status", "=", 0)
