...
class Reports extends Controller
{
      ...
      $lg_lists =  DB::table('account_ledgers')
        ...
        foreach ($lg_lists as $key => $value) {
          ...
          $cr_sum = DB::table('account_transactions')
            ...
            ->where("type", "=", 'cr')
            ->where("delete_status", "=", 0)
            ->whereBetween('added_date', [$first_date, $second_date])
            ->sum('amount');
            ...
            ->sum('amount');
            $balance = $dr_sum - $cr_sum;
            $user_arr[] = array($value->ledger,$dr_sum,$cr_sum,$balance);
        }
        return response()->json($user_arr);
    }
    public function ledger_report(){
      ...
      $currency_lists = crny::all();
		  ...
		  $data = ['lg_lists'=> $lg_lists,'currency_lists' => $currency_lists];
      return view('accounts.reports.ledger_report',$data); 
  }
  ...
  public function stock_report(){
    ...
    return view('accounts.reports.stock_report',$data); 
  }
  ...
  public function sales_report(){
    ...
    return view('accounts.reports.sales_report',$data); 
  }
  ...
  public function purchase_report(){
    ...
    $data = ['pc_lists'=> $pc_lists,'currency_lists' => $currency_lists,'lg_lists' =>$lg_lists];
    return view('accounts.reports.purchase_report',$data); 
  }
...
return view('accounts.reports.pending_sales_invoices',$data)->with('no', 1); 
  }
...
public function change_password_account(Request $request, string $id)
{
...
DB::connection('mysql_second')->table('users')->where('id', $id)
  ...
  return redirect()->route('account_change_password')->with('success', 'Password changed successfully.');
