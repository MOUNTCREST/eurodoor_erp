...
class Receipt extends Controller
{
    ...
    {
		return view('accounts.receipt.index');  
    }
    public function get_receipt_list_account(){
      ...
      ->where("receipts.department", "=",'Account')
      ->get();
  ...
  foreach ($result as $r) {
      ...
      $editButton = '<a href="' . route('receipt_account_edit',$r->id ) . '"> <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-4.5 h-4.5">
      <path
          opacity="0.5"
  ...
  $btns = '<form method="post" action="'. route('receipt_account_delete', $r->id) .'">'. csrf_field() . method_field('DELETE') .'<div class="flex gap-4 items-center">'.$editButton.$deleteButton.'</div></form>';
 ...
 $ref_no =$r->reference_no;
    ...
    public function create()
    {
        ...
        return view('accounts.receipt.create',$data);
    }
  ...
  if(request('type') == 'black'){
   ...
   DB::connection('mysql_second')->table('receipts')->insert([
     ...
     'updated_at' => DB::raw('NOW()'),
        ...
        return redirect(route('receipt_account_list'));
    }
    ...
    public function edit(rcpt $id)
    {
        ...
        return view('accounts.receipt.edit',compact('result','lg_lists','lg_lists_b_c','currency_lists'));
