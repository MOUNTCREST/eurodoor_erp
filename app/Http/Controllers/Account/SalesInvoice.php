...
class SalesInvoice extends Controller
{
    ...
    public function index()
    {
		...
		return view('accounts.sales_invoice.index'); 
    }
    ...
    ->get();
        ...
        return view('accounts.sales_invoice.create',$data); 
    }
...
$endDate = date('Y-m-d', strtotime("$startDate +$due_date_no day"));
    ...
    ->update([
      ...
      return redirect(route('sales_invoice_account_list'));
    }
    ...
    public function show(string $id)
    {
       // $result = $id;
        ...
        return view('accounts.sales_invoice.view',compact('result','s_items'))->with('no', 1);
    }
    ...
    public function edit(string $id)
    {
        ...
        return view('accounts.sales_invoice.edit',compact('result','lg_lists','s_a_lists','currency_lists','customer_lists','pt_lists','s_items'))->with('no', 0);
    }
...
//->setOptions($options)
   ...
   $pdf = PDF::loadView('accounts.sales_invoice.invoice', $data)->setPaper('a5');

    return $pdf->download($result->s_i_no . ".pdf");

        //return view('accounts.sales_invoice.invoice',$data); 
    }
    public function get_sales_invoice_list_in_account(){
        ...
        ->where("sales_invoices.company_id", "=", Session::get('company_id'))
        ->where("sales_invoices.department", "=", "Account")
        ->get();
    ...
    foreach ($result as $r) {
        ...
        $editButton = '<a href="' . route('sales_invoice_account_edit',$r->id ) . '"> <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-4.5 h-4.5">
        <path
            opacity="0.5"
    ...
    $viewButton = '<a href="' . route('sales_invoice_account_view',$r->id ) . '">  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
    <path
        opacity="0.5"
    ...
    $btns = '<form method="post" action="'. route('sales_invoice_account_delete', $r->account_id) .'">'. csrf_field() . method_field('DELETE') .'<div class="flex gap-4 items-center">'.$editButton.$viewButton.$deleteButton.'</div></form>';
    $date = \Carbon\Carbon::parse($r->s_date)->format('d-m-Y');
    $ref_no ='<a style="color:#1212c1;" href="'. route('sales_invoice_account_view', $r->id) .'">'.$r->s_i_no.'</a>';
