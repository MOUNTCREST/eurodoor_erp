...
class PurchaseInvoice extends Controller
{
    ...
    public function index()
    {
		...
		return view('accounts.purchase_invoice.index'); 
    }
    ...
    public function create()
    {
        ...
        $p_a_lists =  DB::table('account_ledgers')
        ->select('account_ledgers.id','account_ledgers.ledger')
        ->where("account_ledgers.account_group_name", "=", "DIRECT EXPENSE")
        ->get();

        $supplier_lists =  DB::table('suppliers')
        ->select('suppliers.*')
        ...
        $data_pi =  DB::table('purchase_invoices')
        ...
        ->get();
         ...
         if(empty($data_pi)){
            ...
            $num = DB::table('purchase_invoices')
                ->where('company_id', '=', Session::get('company_id'))
                ->count();
         }
         $rf_n = $num + 1;
         ...
         $data = ['pt_lists' => $pt_lists,'lg_lists'=> $lg_lists,'currency_lists' => $currency_lists,'r_no' => $r_no,'p_a_lists' => $p_a_lists,'supplier_lists' => $supplier_lists];
        return view('accounts.purchase_invoice.create',$data); 
    }
    ...
    public function store(Request $request)
    {
        $validatedData = $request->validate([
            ...
            'p_i_no.required' => 'The Purchase No Field Is Required.',
            'supplier_id.required' => 'The Supplier Name Field Is Required.',
            'p_date.required' => 'The Date Field Is Required.',
        ]);
       ...
       $data_acnts =  DB::table('accounts')
        ...
        DB::connection('mysql_second')->table('accounts')->insert([
         ...
         DB::transaction(function () use ($account_id,$total_w) {
            DB::connection('mysql')->table('account_transactions')->insert([
              'added_date' =>  request('p_date'),
              'account_id' => $account_id,
              'ledger_id' =>  request('supplier_id'),
              'type' =>  'cr',
              'currency_id' =>request('currency_id'),
              'amount' =>  request('g_amount_net_total'),
              'narration' =>  request('narration'),
           ...
           DB::connection('mysql')->table('account_transactions')->insert([
               'added_date' =>  request('p_date'),
               'account_id' => $account_id,
               'ledger_id' =>  request('purchase_account_id'),
               'type' =>  'dr',
               'currency_id' =>request('currency_id'),
               'amount' =>  request('g_amount_net_total'),
               'narration' =>  request('narration'),
        ...
        $p_invoice_id = DB::connection('mysql')->table('purchase_invoices')->insertGetId([
            'ref_no' => request('ref_no'),
            'p_i_no' =>  request('p_i_no'),
          ...
          DB::connection('mysql_second')->table('purchase_invoices')->insert([
            'ref_no' => request('ref_no'),
            'p_i_no' =>  request('p_i_no'),
            'p_date' => request('p_date'),
            'account_id' => $account_id,
     ...
     DB::connection('mysql_second')->table('black_or_whites')->insert([
      ...
      'updated_at' => DB::raw('NOW()'),
       ...
       for($i=0;$i<count($item_id);$i++)
          {
            if ($item_id[$i] != null) {
            DB::transaction(function () use ($p_invoice_id, $item_id, $item_unit, $qty, $white_amount,$gst,$black_amount,$amount,$i) {
                $item_details = prdct::where('id',$item_id[$i])->first();

                $unit_price =  $white_amount[$i] + $gst[$i] + $black_amount[$i];
                $unit_price2 =  $white_amount[$i] + $gst[$i] ;
                DB::connection('mysql')->table('purchase_invoice_items')->insert([
                    'pi_id' =>  $p_invoice_id,
                    'added_date' => request('p_date'),
                ...
                DB::connection('mysql_second')->table('purchase_invoice_items')->insert([
                    'pi_id' =>  $p_invoice_id,
                    'added_date' => request('p_date'),
                    'item_category' => $item_details->category_id,
                    'item_unit' =>  $item_unit[$i],
      ...
      return redirect(route('purchase_invoice_account_list'));
    }
    ...
    public function show(string $id)
    {
        ...
        return view('accounts.purchase_invoice.view',compact('result','p_items'))->with('no', 1);
    }
    ...
    {
        $result = DB::table('purchase_invoices')
        ->select('purchase_invoices.*','suppliers.supplier_name','suppliers.permenant_address')
        ->join('suppliers','purchase_invoices.supplier_id','=','suppliers.ledger_id')
        ->where("purchase_invoices.id", "=", $id)
        ->first();
        ...
        $p_items = DB::table('purchase_invoice_items')
        ->select('purchase_invoice_items.*','units.unit_name')
        ->join('units','purchase_invoice_items.item_unit','=','units.id')
        ->where("purchase_invoice_items.pi_id", "=", $id)
        ->get();
        ...
        return view('accounts.purchase_invoice.edit',compact('result','lg_lists','p_a_lists','currency_lists','pt_lists','p_items','supplier_lists'))->with('no', 0);
    }
    ...
    DB::connection('mysql_second')->table('accounts')->where('id', $id)
        ...
        // Find the record to update in database1
    $record1 = DB::connection('mysql')->table('purchase_invoices')->where('account_id', $account_id)->first();

    // Update the record with the new data
    DB::connection('mysql')->table('purchase_invoices')->where('account_id', $account_id)
        ->update([
            'p_i_no' =>  request('p_i_no'),
            'p_date' =>  request('p_date'),
            'supplier_id' =>request('supplier_id'),
    ...
    // Find the record to update in database2
    $record2 = DB::connection('mysql_second')->table('purchase_invoices')->where('account_id', $account_id)->first();

    // Update the record with the new data
    DB::connection('mysql_second')->table('purchase_invoices')->where('account_id', $id)
        ->update([
          'p_i_no' =>  request('p_i_no'),
          'p_date' =>  request('p_date'),
          'supplier_id' =>request('supplier_id'),
        ...
        $p_invoice_id = DB::table('purchase_invoices')
       ...
       \DB::connection('mysql_second')->table('account_transactions')->where('account_id', $account_id)->delete();
        ...
        // Delete record from table1 in db1
       \DB::connection('mysql')->table('purchase_invoice_items')->where('pi_id', $p_invoice_id)->delete();

          // Delete record from table1 in db2
        \DB::connection('mysql_second')->table('purchase_invoice_items')->where('pi_id', $p_invoice_id)->delete();

          // Delete record from table1 in db1
       \DB::connection('mysql')->table('stocks')->where('p_id', $p_invoice_id)->delete();

       // Delete record from table1 in db2
     \DB::connection('mysql_second')->table('stocks')->where('p_id', $p_invoice_id)->delete();
 ...
 DB::connection('mysql')->table('black_or_whites')->where('account_id', $account_id)
     ->update([
      ...
      // Update the record with the new data
 DB::connection('mysql')->table('purchase_invoice_items')->where('pi_id', $p_invoice_id)
 ->update([
     'delete_status' => 1,
     'updated_at' => DB::raw('NOW()'),
 ]);
 ...
 DB::connection('mysql')->table('purchase_invoice_items')->where('pi_id', $p_invoice_id)
     ->update([
       ...
       // Update the record with the new data
 DB::connection('mysql')->table('stocks')->where('p_id', $p_invoice_id)
 ->update([
     'delete_status' => 1,
     'updated_at' => DB::raw('NOW()'),
 ]);
 // Update the record with the new data
 DB::connection('mysql_second')->table('stocks')->where('p_id', $p_invoice_id)
     ->update([
         'delete_status' => 1,
         'updated_at' => DB::raw('NOW()'),
     ]);

     return redirect(route('purchase_account_invoice_list'));
    }
    public function get_unit_of_item(Request $request){
        $item_id = $request->item_id;
        ...
        return response()->json(['unit_id' =>$item_details->id,'unit_name' =>$item_details->unit_name]);
    }
    public function get_view_data(Request $request){
        $p_invoice_id = $request->p_invoice_id;

        $purchase_invoice = DB::table('purchase_invoices')
        ->select('purchase_invoices.*','currencies.code','supplier.ledger as supplier_ledger','p_account_ledger.ledger as purchase_account')
        ->join('currencies','purchase_invoices.currency_id','=','currencies.id')
        ->join('account_ledgers as p_account_ledger', 'purchase_invoices.purchase_account_id', '=', 'p_account_ledger.id')
        ->join('account_ledgers as supplier', 'purchase_invoices.supplier_id', '=', 'supplier.id')
        ...
        $purchase_invoice_items = DB::table('purchase_invoice_items')
        ->select('purchase_invoice_items.*','units.unit_name','products.product_name')
        ->join('units','purchase_invoice_items.item_unit','=','units.id')
        ->join('products','purchase_invoice_items.item_id','=','products.id')
        ->where("purchase_invoice_items.pi_id", "=", $p_invoice_id)
        ->get();

        return response()->json(['purchase_invoice' =>$purchase_invoice,'purchase_invoice_items' =>$purchase_invoice_items]);
    }
    public function get_supplier_address(Request $request){
        $supplier_ledger_id = $request->supplier_id;


        $supplier_address = DB::table('suppliers')
        ->where('ledger_id', $supplier_ledger_id)
        ->value('permenant_address');

        return response()->json(['supplier_address' =>$supplier_address]);

    }
    public function save_supplier_details(Request $request){
        ...
        ->where('account_group_name', 'SUNDARY CREDITORS')
        ->value('id');
        ...
        $ledger_id_one = DB::connection('mysql')->table('account_ledgers')->insertGetId([
            ...
            'account_group_name' =>  'SUNDARY CREDITORS',
            'company_id' => Session::get('company_id'),
            'created_at' => DB::raw('NOW()'),
            'updated_at' => DB::raw('NOW()'),
        ]);
        ...
        DB::connection('mysql')->table('suppliers')->insert([
        ...
        DB::connection('mysql_second')->table('suppliers')->insert([
            'supplier_name' =>  $supplier_name,
            'mobile_no' => $mobile_no,
    ...
    public function get_supplier_list(){
        ...
        return response()->json(['supplier_lists' =>$supplier_lists]);
    }
    public function get_purchase_invoice_list_in_account(){
        ...
        ->where("purchase_invoices.company_id", "=", Session::get('company_id'))
        ->where("purchase_invoices.department", "=", "Account")
        ->get();
    ...
    foreach ($result as $r) {
        ...
        $editButton = '<a href="' . route('purchase_invoice_account_edit',$r->id ) . '"> <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-4.5 h-4.5">
        <path
            opacity="0.5"
    ...
    $viewButton = '<a href="' . route('purchase_invoice_account_view',$r->id ) . '">  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
    <path
        opacity="0.5"
    ...
    $btns = '<form method="post" action="'. route('purchase_invoice_delete_in_account', $r->account_id) .'">'. csrf_field() . method_field('DELETE') .'<div class="flex gap-4 items-center">'.$editButton.$viewButton.$deleteButton.'</div></form>';
    $date = \Carbon\Carbon::parse($r->p_date)->format('d-m-Y');
    $ref_no ='<a style="color:#1212c1;" href="'. route('purchase_invoice_account_view', $r->id) .'">'.$r->ref_no.'</a>';
